...
- **Resolution**: The code is written to expect the *correct* structure (`portraits`, nested folders). In the current environment, this will result in empty galleries or missing modal images. **This is expected behavior** until the filesystem is updated to match the authoritative structure.
